...
def access_state():
    if _secret_bool("PREMIUM_PREVIEW", False):
        return {"allowed": True, "mode": "preview", "email": "local-preview", "name": "Owner Preview"}
    ...
    if not logged:
        return {"allowed": False, "mode": "login", "email": "", "name": ""}

    email = str(getattr(st.user, "email", "") or "").strip().lower()
    name = str(getattr(st.user, "name", "") or email or "Member")
    members = {str(x).strip().lower() for x in (_secret("member_emails", []) or [])}
    domains = {str(x).strip().lower().lstrip("@") for x in (_secret("member_domains", []) or [])}
    domain = email.split("@", 1)[1] if "@" in email else ""
    allowed = email in members or domain in domains
    return {"allowed": allowed, "mode": "member" if allowed else "denied", "email": email, "name": name}
...
def render_access_gate():
    ...
    <div class="member-lock">
      ...
      <div>
        ...
        <p>The automatic engine is restricted to approved members. The free Insight Engine remains available for manual inputs.</p>
      </div>
    </div>
    ''')
    ...
    if access["mode"] == "login":
        try:
            st.button("Sign in to Member Access", type="primary", on_click=st.login)
        except Exception:
            st.warning("OIDC member login is not configured yet. See MEMBER_SETUP.md in the V7 package.")
    else:
        st.error("This signed-in account is not on the member access list.")
        try:
    ...
    with st.expander("Owner setup"):
        st.code('''member_emails = ["member@example.com"]
...
def yf_download(symbol, interval, period):
    try:
        try:
            ...
            pass
        try:
            d = yf.download(symbol, interval=interval, period=period, auto_adjust=False, progress=False, threads=False, repair=True)
        except Exception:
            d = yf.download(symbol, interval=interval, period=period, auto_adjust=False, progress=False, threads=False)
        return _norm_df(d)
    except Exception:
        return pd.DataFrame()
...
def auto_timeframes(asset):
    ...
    five, fs = fetch_asset(asset, "5m", "30d")
    one, os = fetch_asset(asset, "1m", "5d")
    frames = {
        "Monthly": resample_ohlc(daily, "ME"), "Weekly": resample_ohlc(daily, "W-FRI"), "Daily": daily,
        "H4": resample_ohlc(hourly, "4h"), "H1": hourly, "M30": resample_ohlc(five, "30min"),
        "M15": resample_ohlc(five, "15min"), "M5": five, "M1": one,
    }
    src = {"Monthly": ds, "Weekly": ds, "Daily": ds, "H4": hs, "H1": hs, "M30": fs, "M15": fs, "M5": fs, "M1": os}
    out = {}
    for tf in TIMEFRAMES:
        out[tf] = classify_tf(frames[tf])
        out[tf]["source"] = src[tf]
        out[tf]["last_timestamp"] = str(frames[tf].index[-1]) if not frames[tf].empty else "Unavailable"
    return out
...
def analyze(asset,style,session,tf_details,drivers):
    tfs={tf:tf_details[tf]["state"] for tf in TIMEFRAMES}; w=STYLE_TF_WEIGHTS[style]
    ps=sum(DIR[tfs[tf]]*w[tf] for tf in TIMEFRAMES)
    tw=sum(r["weight"] for r in drivers); ds=sum(r["weighted_effect"] for r in drivers)/tw if tw else 0
    pw,dw=STYLE_PRICE_DRIVER_WEIGHTS[style]; combined=ps*pw+ds*dw
    sm=session_tape(asset,session)
    if sm and style=="Session Trader": combined += 0.05*sm["direction"]
    combined=float(np.clip(combined,-1,1))
    bd=1 if combined>=0.20 else -1 if combined<=-0.20 else 0; bias="LOOK FOR BUYS" if bd>0 else "LOOK FOR SELLS" if bd<0 else "WAIT / NO CLEAR EDGE"
    lt=group_score(tfs,["Monthly","Weekly"]); ht=group_score(tfs,["Daily","H4"]); intr=group_score(tfs,["H1","M30"]); ex=(0.4*DIR[tfs["M15"]]+0.4*DIR[tfs["M5"]]+0.2*DIR[tfs["M1"]])
    if bd==0: ready=0; rlabel="WAIT — BIAS NOT CLEAR"
    else:
        rel=STYLE_EXEC_TFS[style]; aligned=sum(1 if DIR[tfs[t]]==bd else 0.35 if DIR[tfs[t]]==0 else 0 for t in rel); ready=round(100*aligned/len(rel))
        if sm and style in {"Day Trader","Session Trader"}: ready=min(100,ready+8) if sm["direction"]==bd else max(0,ready-12) if sm["direction"]==-bd else ready
        conflict=ps*ds<-0.18; rlabel="FAVOR SETUPS — ALIGNMENT PRESENT" if ready>=70 and not conflict else "WAIT FOR CONFIRMATION" if ready>=45 else "LOW READINESS — STRUCTURAL CONFLICT"
    return {"asset":asset,"style":style,"session":session,"tfs":tfs,"drivers":drivers,"price_score":ps,"driver_score":ds,"combined":combined,"price_strength":round(ps*100),"driver_strength":round(ds*100),"strength":round(abs(combined)*100),"bias":bias,"bias_dir":bd,"readiness":rlabel,"readiness_score":ready,"lt":state_from_score(lt),"ht":state_from_score(ht),"intraday":state_from_score(intr),"execution":state_from_score(ex,0.25),"driver_environment":driver_env(ds),"session_meta":sm}
...
def member_live_insight_page():
    ...
    st.info("Development data adapter: Yahoo Finance via yfinance plus FRED for selected macro series. Replace it with a licensed commercial data provider before selling or redistributing live data to members.")
    ...
    with st.spinner(f"Reading {asset} across 9 timeframes and evaluating drivers..."):
        auto_tf=auto_timeframes(asset); auto_drv=auto_drivers(asset,style)
    if sum(1 for x in auto_tf.values() if x.get("bars",0)<20)>=5:
        st.error("The development market-data provider did not return enough data for a reliable 9-timeframe analysis. Try again later or use the free manual engine."); return
    tf,drv=apply_overrides(auto_tf,auto_drv); r=analyze(asset,style,session,tf,drv); confirm,weaken=confirm_lists(r)
    ...
    rows="";
    for t in TIMEFRAMES:
        d=tf[t]; cls="positive" if d["state"]=="Bullish" else "negative" if d["state"]=="Bearish" else "neutral"; rows+=f'<tr><td><strong>{t}</strong></td><td class="{cls}">{d["state"]}</td><td>{d.get("score",0):+.2f}</td><td>{d.get("structure","")}</td><td>{d.get("trend","")}</td><td>{d.get("momentum","")}</td></tr>'
    ...
    if access["mode"]!="preview":
        try: st.button("Sign out of Member Access",on_click=st.logout)
