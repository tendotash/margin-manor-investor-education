...
SPECIAL_MARKET_ALIASES = {
    "NQ": "NAS100 — Nasdaq 100",
    "NQ1": "NAS100 — Nasdaq 100",
    "MNQ": "NAS100 — Nasdaq 100",
    "MNQ1": "NAS100 — Nasdaq 100",
...
def insight_engine_page():
    ...
    with c1:
        ...
        if pair_mode == "Enter Any Pair":
            ...
            raw_asset = st.selectbox(
                "Other Market",
                ["NQ", "ES", "NAS100", "SPX500", "BTCUSD", "WTI"],
...
with st.sidebar:
    ...
    if member_access["allowed"]:
        member_name = member_access.get("name") or member_access.get("email") or "Member"
        member_email = member_access.get("email") or ""
        member_plan = member_access.get("plan") or "Member"

        render_html(f"""
        <div class="sidebar-member-card active">
            <div class="sidebar-member-badge">ACTIVE MEMBER</div>
            <div class="sidebar-member-name">{member_name}</div>
            <div class="sidebar-member-plan">{member_plan}</div>
            <div class="sidebar-member-email">{member_email}</div>
        </div>
        """)
        ...
        <div class="sidebar-member-card">
            <div class="sidebar-member-badge">MEMBERS</div>
            <div class="sidebar-member-name">Live Insight Engine</div>
            <div class="sidebar-member-copy">
                Sign in with your Margin Manor member email and password.
            </div>
        </div>
        """)

        with st.form("sidebar_member_login_form", clear_on_submit=False):
            login_email = st.text_input("Email", key="sidebar_member_email", placeholder="member@email.com")
            login_password = st.text_input("Password", type="password", key="sidebar_member_password")
            login_submit = st.form_submit_button("Member Login", type="primary", use_container_width=True)
        ...
        with st.expander("Forgot password?"):
            reset_email = st.text_input("Member email", key="sidebar_reset_email", placeholder="member@email.com")
            if st.button("Send Reset Email", key="sidebar_reset_btn", use_container_width=True):
                ok, message = send_password_reset(reset_email)
                (st.success if ok else st.error)(message)

        st.caption("No public registration. Member accounts are issued by Margin Manor.")

    elif member_access["mode"] == "setup_required":
        render_html("""
        <div class="sidebar-member-card setup">
            <div class="sidebar-member-badge">MEMBER LOGIN SETUP</div>
            <div class="sidebar-member-name">Connect Supabase</div>
            <div class="sidebar-member-copy">
                Add the Supabase URL and publishable key in Streamlit Cloud Secrets.
            </div>
        </div>
        """)
        ...
        render_html(f"""
        <div class="sidebar-member-card denied">
            <div class="sidebar-member-badge">SIGNED IN</div>
            <div class="sidebar-member-name">Membership Required</div>
            <div class="sidebar-member-copy">
                {member_access.get("message", "This account does not currently have active member access.")}
            </div>
            <div class="sidebar-member-email">{member_access.get("email", "")}</div>
        </div>
        """)
    ...
    for key in ["Learn", "Briefs", "Tools", "Members", "Contact"]:
    ...
    # Direct member entry:
    # After a successful Supabase login, an active member is automatically taken
    # to the Member Live Insight Engine. They can navigate normally after that.
    if member_access["allowed"]:
        if st.session_state.pop("_mm_force_member_page", False):
            page = "Member Live Insight Engine"
...
elif page == "Member Live Insight Engine":
    member_live_insight_page()
