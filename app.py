...
from premium_engine import member_live_insight_page, access_state
...
with st.sidebar:
    ...
    # Persistent cloud member-login area.
    ...
    if member_access["allowed"]:
        ...
        if st.button(
            "Open Live Insights",
            key="sidebar_open_member_engine",
            type="primary",
        ...
        <div class="sidebar-member-card">
            ...
            <div class="sidebar-member-copy">
                Sign in with your approved Google account for automatic market analysis.
            </div>
        </div>
        """)
        try:
            if st.button(
                "Member Login",
                key="sidebar_member_login",
                type="primary",
                use_container_width=True,
            ):
                st.login()
        except Exception:
            st.caption("Member login will activate after Google OIDC is added in Streamlit Cloud Secrets.")
        ...
        <div class="sidebar-member-card denied">
            ...
            <div class="sidebar-member-copy">
                {member_access.get("email", "")} is not currently on the approved member list.
            </div>
        </div>
        """)
        try:
    ...
    # Direct member entry:
    # st.login() returns to the app homepage in a fresh session. On the first
    # authenticated render, an approved member is automatically taken to the
    # Member Live Insight Engine. They can navigate normally after that.
    if member_access["allowed"]:
